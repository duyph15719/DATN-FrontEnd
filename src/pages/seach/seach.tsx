...
const Seach = (props: Props) => {
    ...
    const { productSearch } = useAppSelector(state => state.ProductReducer)
    const { categories } = useAppSelector((state: any) => state.CategoriesReducer)
    const [currentPage, setCurrentPage] = useState(1);
    let key = searchParams.get('q');
    const postPerPage = 9;
    const totalPosts = productSearch.length;
    const indexOfLastPost = productSearch * postPerPage;
    const indexOfFirstPost = indexOfLastPost - postPerPage;
    const filterPosts = productSearch.slice(indexOfFirstPost, indexOfLastPost);

    const postPerPage1 = 5;
    const totalPosts1 = productSearch.length;
    const indexOfLastPost1 = currentPage * postPerPage1;
    const indexOfFirstPost1 = indexOfLastPost1 - postPerPage1;
    const filterPosts1 = productSearch.slice(indexOfFirstPost1, indexOfLastPost1);
    return (
        <div>
            <div className="nav-product max-w-7xl mx-auto ">
                ...
                <div className='flex flex-col md:flex-row ' >
                    ...
                    <div className="product-aside-right w-[100%] md:w-[100%] grid md:grid-cols-4 grid-cols-2 gap-4 pt-10 ">
                        {productSearch != "" ? productSearch?.map((item: any) => (
                            <>
                        ...
                        )) : <div className='text-center my-32 text-2xl '>Không có sản phẩm phù hợp với từ khóa " <span className='font-bold'>{key}</span> "</div>}
